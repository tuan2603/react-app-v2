export const LOG_IN = 'LOGIN';
export const LOG_OUT = 'LOGOUT';
export const SHOW_NOTIFICATION = 'SHOW_NOTIFICATION';
export const HIDE_NOTIFICATION = 'HIDE_NOTIFICATION';
export const TOGGLE_IS_ADDING = 'TOGGLE_IS_ADDING';
export const ADD_ITEM = 'ADD_ITEM';
export const REMOVE_ITEM = 'REMOVE_ITEM';
export const UPDATE_INTRODUCE = 'UPDATE_INTRODUCE';
export const REMOVE_INTRODUCE = 'REMOVE_INTRODUCE';
export const ADD_ITEM_CATEGORY = 'ADD_ITEM_CATEGORY';
export const REMOVE_ITEM_CATEGORY = 'REMOVE_ITEM_CATEGORY';

export const LOAD_CATS_SUCCESS = 'LOAD_CATS_SUCCESS';
export const UPDATE_CAT_SUCCESS = 'UPDATE_CAT_SUCCESS';
export const CREATE_CAT_SUCCESS = 'CREATE_CAT_SUCCESS';
export const DELETE_CAT_SUCCESS = 'DELETE_CAT_SUCCESS';

export const LOAD_ADV_SUCCESS = 'LOAD_ADV_SUCCESS';
export const UPDATE_ADV_SUCCESS = 'UPDATE_ADV_SUCCESS';
export const CREATE_ADV_SUCCESS = 'CREATE_ADV_SUCCESS';
export const DELETE_ADV_SUCCESS = 'DELETE_ADV_SUCCESS';

export const LOAD_Q_SUCCESS = 'LOAD_Q_SUCCESS';
export const UPDATE_Q_SUCCESS = 'UPDATE_Q_SUCCESS';
export const CREATE_Q_SUCCESS = 'CREATE_Q_SUCCESS';
export const DELETE_Q_SUCCESS = 'DELETE_Q_SUCCESS';

export const LOAD_PAGES_SUCCESS = 'LOAD_PAGES_SUCCESS';
export const UPDATE_PAGE_SUCCESS = 'UPDATE_PAGE_SUCCESS';
export const CREATE_PAGE_SUCCESS = 'CREATE_PAGE_SUCCESS';
export const DELETE_PAGE_SUCCESS = 'DELETE_PAGE_SUCCESS';

export const LOAD_TESTIMONIAL_SUCCESS = 'LOAD_TESTIMONIAL_SUCCESS';
export const UPDATE_TESTIMONIAL_SUCCESS = 'UPDATE_TESTIMONIAL_SUCCESS';
export const CREATE_TESTIMONIAL_SUCCESS = 'CREATE_TESTIMONIAL_SUCCESS';
export const DELETE_TESTIMONIAL_SUCCESS = 'DELETE_TESTIMONIAL_SUCCESS';

export const CAT_IS_EDIT = 'CAT_IS_EDIT';
